...
uv run torchrun --standalone --nproc_per_node=$NPROC \
    ...
    --weight_decay 0.01 \
    --batch_size 8 \
    --encoder_ckpt ../ecg_encoder/src/runs/pretrain/st_mem/1/checkpoints/epoch_best.pt \
    --grad_accum_steps 2 \
    --num_workers 16 \
    --torch_compile \
    --ref_global_bs 128
...
uv run torchrun --standalone --nproc_per_node=$NPROC \
    ...
    "${COMMON_FLAGS[@]}" \
  --train_phase sft \
  --data ecg-qa-ptbxl-250-2500 ecg-qa-mimic-iv-ecg-250-2500 pretrain-mimic-250-2500 ecg-instruct-45k-250-2500 \
...
uv run torchrun --standalone --nproc_per_node=$NPROC \
    ...
    "${COMMON_FLAGS[@]}" \
  --train_phase sft \
  --data ecg-qa-ptbxl-250-2500 ecg-qa-mimic-iv-ecg-250-2500 pretrain-mimic-250-2500 ecg-instruct-45k-250-2500 ecg-grounding-250-2500 ecg-instruct-ecg-r1 base-ecg-r1 ecg-qa-cot \
  --update encoder connector llm \
  --optimizer muon \
  --lr 2e-4 \
